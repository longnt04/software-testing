...
@pytest.mark.parametrize("mana,attack,expected", load_test_cases("single_boundary.csv"))
def test_single_boundary(mana, attack, expected):
...
@pytest.mark.parametrize("mana,attack,expected", load_test_cases("decision_table.csv"))
def test_decision_table(mana, attack, expected):
    result = get_wizard_title(mana, attack)
    print(f"| result={result}, expected={expected}")
    assert result == expected, f"Failed for mana={mana}, attack={attack}: got {result}, expected {expected}"
